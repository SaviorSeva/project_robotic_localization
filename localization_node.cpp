...
void initialize_localization() {
    ...
    min_orientation = -175 * M_PI / 180;			// Lowest angle is -175 degree
    max_orientation = M_PI;							// Highest angle is 180 degree
		...
		for(float y = min_y; y <= max_y; y+=0.05){
			if ( cell_value(x, y) == 0 ) { // robair can only be at a free cell
				for(float rad = min_orientation; rad <= max_orientation; rad += (5 * M_PI / 180)){
				    int score_current = sensor_model(x, y, rad);
				    ...
				    if( score_current > score_max ){
				    	...
				    	estimated_position.y = y;
				    	estimated_orientation = rad;
				    }
				}
			}
		}
	}
	ROS_INFO("LOCATION WITH THE BEST SCORE(%f, %f, %f), score = %i", estimated_position.x, estimated_position.y, estimated_orientation, score_max);
...
// initialize_localization should work before you implement this method
    ...
    min_orientation = predicted_orientation - M_PI/6;			// Lowest angle is -30 degree
    max_orientation = predicted_orientation + M_PI/6;			// Highest angle is 30 degree
	...
	ROS_INFO("ESTIMATION WITH THE BEST SCORE(%f, %f, %f), score = %i", estimated_position.x, estimated_position.y, estimated_orientation, score_max);
	sensor_model(estimated_position.x, estimated_position.y, estimated_orientation);
	populateMarkerTopic();
